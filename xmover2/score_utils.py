...
def collate_idf(arr, tokenize, numericalize, idf_dict,
    ...
    tokens = [["[CLS]"]+tokenize(a)+["[SEP]"] for a in arr][:512]
...
def lm_perplexity(model, hyps, tokenizer, batch_size=1, device='cuda:0'):
    ...
    for batch_start in range(0, len(hyps), batch_size):
        ...
        if len(tokenize_input) <=1:
            ...
            if len(tokenize_input) > 511:
                tokenize_input = tokenize_input[:511]
...
def word_mover_score( projection, bias, model, tokenizer, src, hyps,  batch_size=256, device='cuda:0'):
    idf_dict_src = defaultdict(lambda: 1.)
    idf_dict_hyp = defaultdict(lambda: 1.)
    
    preds = defaultdict(lambda: defaultdict(lambda: []))
    for batch_start in range(0, len(src), batch_size):
        batch_src = src[batch_start:batch_start+batch_size]
        batch_hyps = hyps[batch_start:batch_start+batch_size]
        
        src_embedding_orig, src_lens, src_masks, src_idf, src_tokens = get_bert_embedding(batch_src, model, tokenizer, idf_dict_src,
                                       device=device)
        hyp_embedding_orig, hyp_lens, hyp_masks, hyp_idf, hyp_tokens = get_bert_embedding(batch_hyps, model, tokenizer, idf_dict_hyp,
                                       device=device)

        for mapping in ["CLP", "UMD"]:
            for n_gram in [1,2]:
                src_embedding = src_embedding_orig[-1]
                hyp_embedding = hyp_embedding_orig[-1]
                ...
                for i in range(batch_size):
                    ...
                    preds[mapping][n_gram].append(1 - score)
